...
class GRNParser(BaseParser):
  ...
  def __call__(self, dataset, moving_params=None):
    ...
    in_neigh_idx = dataset.in_neighbor # [batch, word, neigh]
    in_neighbor_rel = dataset.in_neighbor_rel # [batch, word, neigh]
    in_neigh_mask = tf.to_float(dataset.in_neighbor_mask)  # [batch, word, neigh]
    out_neigh_idx = dataset.out_neighbor  # [batch, word, neigh]
    out_neighbor_rel = dataset.out_neighbor_rel  # [batch, word, neigh]
    out_neigh_mask = tf.to_float(dataset.out_neighbor_mask)

    reuse = (moving_params is not None)
    sequence_mask = tf.expand_dims(tf.to_float(tf.greater_equal(inputs[:, :, 0], vocabs[0].ROOT)), 2)
    ...
    #TODO graph encoding
    in_neigh_rel_repre = vocabs[2].embedding_lookup(in_neighbor_rel, moving_params=self.moving_params)
    out_neigh_rel_repre = vocabs[2].embedding_lookup(out_neighbor_rel, moving_params=self.moving_params)
    _, grn_top_recur, _ = GraphEncoder.call(top_recur, sequence_mask,
            in_neigh_idx, in_neigh_rel_repre, in_neigh_mask, out_neigh_idx, out_neigh_rel_repre, out_neigh_mask,
            self.word_keep_prob, self.num_syntax_match_layer, moving_params)

    if self.use_feature_residual:
      top_recur = top_recur + grn_top_recur
    else:
      top_recur = grn_top_recur
