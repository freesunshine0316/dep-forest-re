...
class GraphEncoder(object):
    ...
    def call(cls, node_repre, node_mask, in_neigh_idx, in_neigh_rel_repre, in_neigh_mask,
        ...
        with tf.variable_scope('grn_encoder', reuse=reuse):
            # =========== in neighbor
            # [batch_size, passage_len, neigh_size_max, node_dim]
            in_neigh_node_repre = collect_neighbor_node_representations(node_repre, in_neigh_idx)

            # [batch_size, passage_len, neigh_size_max, node_dim + rel_dim]
            in_neigh_repre = tf.concat(3, [in_neigh_node_repre, in_neigh_rel_repre])
            in_neigh_repre = tf.multiply(in_neigh_repre, tf.expand_dims(in_neigh_mask, axis=-1))
            # [batch_size, passage_len, node_dim + rel_dim]
            in_neigh_repre = tf.reduce_sum(in_neigh_repre, axis=2)

            # ============ out neighbor
            # [batch_size, passage_len, neigh_size_max, node_dim]
            out_neigh_node_repre = collect_neighbor_node_representations(node_repre, out_neigh_idx)

            # [batch_size, passage_len, neigh_size_max, node_dim + rel_dim]
            out_neigh_repre = tf.concat(3, [out_neigh_node_repre, out_neigh_rel_repre])
            out_neigh_repre = tf.multiply(out_neigh_repre, tf.expand_dims(out_neigh_mask, axis=-1))
            # [batch_size, passage_len, node_dim + rel_dim]
            out_neigh_repre = tf.reduce_sum(out_neigh_repre, axis=2)

            # =====transpose neighbor_representations
            trans_dim = node_dim

            w_trans = tf.get_variable("w_trans", [node_dim+rel_dim, trans_dim], dtype=tf.float32)
            w_trans = add_matrix_to_moving_average(w_trans, moving_params)

            b_trans = tf.get_variable("b_trans", [trans_dim], dtype=tf.float32)
            b_trans = add_bias_to_moving_average(b_trans, moving_params)

            in_neigh_repre = tf.reshape(in_neigh_repre, [-1, node_dim+rel_dim])
            in_neigh_repre = tf.matmul(in_neigh_repre, w_trans) + b_trans
            in_neigh_repre = tf.tanh(in_neigh_repre)

            out_neigh_repre = tf.reshape(out_neigh_repre, [-1, node_dim+rel_dim])
            out_neigh_repre = tf.matmul(out_neigh_repre, w_trans) + b_trans
            out_neigh_repre = tf.tanh(out_neigh_repre)

            # assume each node has a neighbor vector, and it is None at the beginning
            node_hidden = tf.zeros([batch_size, node_size_max, node_dim])
            node_cell = tf.zeros([batch_size, node_size_max, node_dim])

            w_in_ingate = tf.get_variable("w_in_ingate", [trans_dim, node_dim], dtype=tf.float32)
            w_in_ingate = add_matrix_to_moving_average(w_in_ingate, moving_params)
            ...
            w_out_ingate = tf.get_variable("w_out_ingate", [trans_dim, node_dim], dtype=tf.float32)
            w_out_ingate = add_matrix_to_moving_average(w_out_ingate, moving_params)
            ...
            w_in_forgetgate = tf.get_variable("w_in_forgetgate", [trans_dim, node_dim], dtype=tf.float32)
            w_in_forgetgate = add_matrix_to_moving_average(w_in_forgetgate, moving_params)
            ...
            w_out_forgetgate = tf.get_variable("w_out_forgetgate", [trans_dim, node_dim], dtype=tf.float32)
            w_out_forgetgate = add_matrix_to_moving_average(w_out_forgetgate, moving_params)
            ...
            w_in_outgate = tf.get_variable("w_in_outgate", [trans_dim, node_dim], dtype=tf.float32)
            w_in_outgate = add_matrix_to_moving_average(w_in_outgate, moving_params)
            ...
            w_out_outgate = tf.get_variable("w_out_outgate", [trans_dim, node_dim], dtype=tf.float32)
            w_out_outgate = add_matrix_to_moving_average(w_out_outgate, moving_params)
            ...
            w_in_cell = tf.get_variable("w_in_cell", [trans_dim, node_dim], dtype=tf.float32)
            w_in_cell = add_matrix_to_moving_average(w_in_cell, moving_params)
            ...
            w_out_cell = tf.get_variable("w_out_cell", [trans_dim, node_dim], dtype=tf.float32)
            w_out_cell = add_matrix_to_moving_average(w_out_cell, moving_params)
            ...
            for i in xrange(num_syntax_match_layer):
                # =============== in edge hidden
                # h_{ij} [batch_size, node_len, neighbors_size, node_dim]
                ...
                in_neigh_prev_hidden = tf.reshape(in_neigh_prev_hidden, [-1, node_dim])
                ...
                out_neigh_prev_hidden = tf.reshape(out_neigh_prev_hidden, [-1, node_dim])

                ## ig
                ingate = tf.sigmoid(tf.matmul(in_neigh_repre, w_in_ingate)
                                          + tf.matmul(in_neigh_prev_hidden, u_in_ingate)
                                          + tf.matmul(out_neigh_repre, w_out_ingate)
                                          + tf.matmul(out_neigh_prev_hidden, u_out_ingate)
                                          + b_ingate)
                ingate = tf.reshape(ingate, [batch_size, node_size_max, node_dim])
                ## fg
                forgetgate = tf.sigmoid(tf.matmul(in_neigh_repre, w_in_forgetgate)
                                          + tf.matmul(in_neigh_prev_hidden, u_in_forgetgate)
                                          + tf.matmul(out_neigh_repre, w_out_forgetgate)
                                          + tf.matmul(out_neigh_prev_hidden, u_out_forgetgate)
                                          + b_forgetgate)
                forgetgate = tf.reshape(forgetgate, [batch_size, node_size_max, node_dim])
                ## og
                outgate = tf.sigmoid(tf.matmul(in_neigh_repre, w_in_outgate)
                                          + tf.matmul(in_neigh_prev_hidden, u_in_outgate)
                                          + tf.matmul(out_neigh_repre, w_out_outgate)
                                          + tf.matmul(out_neigh_prev_hidden, u_out_outgate)
                                          + b_outgate)
                outgate = tf.reshape(outgate, [batch_size, node_size_max, node_dim])
                ## input
                cell_input = tf.tanh(tf.matmul(in_neigh_repre, w_in_cell)
                                          + tf.matmul(in_neigh_prev_hidden, u_in_cell)
                                          + tf.matmul(out_neigh_repre, w_out_cell)
